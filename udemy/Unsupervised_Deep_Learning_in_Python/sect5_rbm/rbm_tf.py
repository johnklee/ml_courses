...
# Course link: https://www.udemy.com/course/unsupervised-deep-learning-in-python/learn/lecture/7339486
...
class RBM(object):
    ...
    def build(self, D, M):
        # params
        self.W = tf.Variable(tf.random.uniform(shape=(D, M)) * np.sqrt(2.0 / M))
        ...
        #     dtype=tf.float32
        # )
        r = tf.random.uniform(shape=tf.shape(p_v_given_h))
        ...
        self.train_op = tf.optimizers.Adam(1e-2).minimize(objective)
